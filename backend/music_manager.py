...
def make_lists_count(sheet_path, music_xml_path, image_path,prev_beats,count,count_l,count_r,prev_beats_l, prev_beats_r):
...
# Iterate over all notes and chords in the score
     total_beats = 0
     list2 = list()
     print(f"time signature{score.getTimeSignatures()[0].numerator}")
     magnifier = score.getTimeSignatures()[0].denominator / 4
     ...
     for i, part in enumerate(score.parts):
        ...
        if(i == 0):
            for element in part.flat.notesAndRests:
                if(isinstance(element.quarterLength,Fraction)):
                    list2.append((element.quarterLength.numerator/element.quarterLength.denominator) *magnifier )
                else:
                    list2.append(element.quarterLength * magnifier)
                    total_beats += element.quarterLength
        elif(i == 1):
            for element in part.flat.notesAndRests:
                if(isinstance(element.quarterLength,Fraction)):
                    beat_list_pianist_l.append((element.quarterLength.numerator/element.quarterLength.denominator) *magnifier )
                else:
                    beat_list_pianist_l.append(element.quarterLength * magnifier)
                    total_beats += element.quarterLength
        else:
            for element in part.flat.notesAndRests:
                if(isinstance(element.quarterLength,Fraction)):
                    beat_list_pianist_r.append((element.quarterLength.numerator/element.quarterLength.denominator) *magnifier )
                else:
                    beat_list_pianist_r.append(element.quarterLength * magnifier)
                    total_beats += element.quarterLength
     ...
     for system in systems:
        ...
        for ele in listsing_:
            ...
            if(x_post  <= prev_x +5 and x_post >= prev_x - 5 ):
                continue
            singer_list.append((ele[0], ele[1],prev_beat +list2[count_s + count], ele[2] ))
            prev_x = x_post
            prev_beat += list2[count + count_s]
            count_s += 1
        ...
        for ele in list_piano_1:
            ...
            if(x_post  <= prev_x +5 and x_post >= prev_x - 5 ):
                continue
            piano_list_l.append((ele[0], ele[1],prev_beat_l + beat_list_pianist_l[count_l + count_piano_l] ))
            prev_x = x_post
            count_piano_l += 1
            prev_beat_l += beat_list_pianist_l[count_l + count_piano_l]
        prev_beat_r = prev_beats_r
        for ele in list_piano_2:
            x_post = ele[0]
            if(x_post  <= prev_x +5 and x_post >= prev_x - 5 ):
                continue
            piano_list_r.append((ele[0], ele[1],prev_beat_r + beat_list_pianist_r[count_r + count_piano_r] ))
     ...
     return (singer_list,piano_list_l, piano_list_r, count_s,count_piano_l,count_piano_r,prev_beat, prev_beat_l, prev_beats_r, template)
class MusicManager():
    def __init__(self, sheet_path, music_xml_path,image_path):
        ...
        for idx, sheet in enumerate(sheet_path):
            #print(total_count)
            (singers,piano_list_l,piano_list_r, count_s,count_l,count_r, prev_beat,prev_beat_l,prev_beat_r, templates) = make_lists_count(sheet,music_xml_path,image_path[idx],beat,total_count,total_count_l,total_count_r,beat_l,beat_r)
            total_count += count_s
            total_count_l += count_l
            total_count_r += count_r
            #print(singers, )
            beat += prev_beat
            beat_l += prev_beat_l
            beat_r += prev_beat_r
            self.singer_lists.append(singers)
            self.piano_lists_l.append(piano_list_l)
            self.piano_lists_r.append(piano_list_r)
